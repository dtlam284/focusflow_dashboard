...
describe('smartLinkingScoring', () => {
  describe('scoreNoteSuggestionForTask', () => {
    ...
    it('scores a note using title and content overlap with human-readable reasons', () => {
      const score = scoreNoteSuggestionForTask(createTask(), createNote())
      ...
      expect(score?.reasons).toContain('Matched title keywords: accessibility')
      expect(score?.reasons).toContain(
        'Matched content keywords: review, dashboard',
      )
    })

    it('adds a recent attachment bonus when present', () => {
      const score = scoreNoteSuggestionForTask(createTask(), createNote(), {
      ...
      expect(score?.reasons).toContain(
        'Recently attached in similar task context',
      )
    })
  ...
  describe('scoreLinkSuggestionForTask', () => {
    ...
    it('scores a link using title and category overlap with human-readable reasons', () => {
      const score = scoreLinkSuggestionForTask(createTask(), createLink())
      ...
      expect(score?.reasons).toContain(
        'Matched title keywords: keyboard, navigation',
      )
      expect(score?.reasons).not.toContain(
        'Matched category keywords: design',
      )
    })
