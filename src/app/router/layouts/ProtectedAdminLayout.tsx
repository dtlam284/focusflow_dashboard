  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white border border-slate-200 rounded-xl p-6 shadow-sm text-center space-y-3">
        ...
        <h1 className="text-base font-semibold text-slate-900">Validating session</h1>
...
export function ProtectedAdminLayout() {
  ...
  const isDevDashboardBypass = import.meta.env.DEV && location.pathname === '/'

  const isDevFeatureBypass =
    import.meta.env.DEV && DEV_BYPASS_PATHS.some((path) => location.pathname.startsWith(path))
  ...
  const roleId = Number(
    roleRecord?.id ?? userRecord.roleId ?? userRecord.role_id ?? userRecord.role,
  )
