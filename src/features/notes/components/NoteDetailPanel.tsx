...
export function NoteDetailPanel() {
    ...
    const isOpen = useAppSelector(selectIsNoteDetailOpen)
    const selectedNoteId = useAppSelector(selectSelectedNoteId)
    const selectedNote = useAppSelector(selectSelectedNoteDetail)

    const relatedTasks = useAppSelector((state) =>
        selectedNoteId ? selectRelatedTasksByNoteId(state, selectedNoteId) : [],
    )
    ...
    //#region effects
    React.useEffect(() => {
        if (isOpen && selectedNoteId && !selectedNote) {
            dispatch(closeNoteDetail())
        }
    }, [dispatch, isOpen, selectedNoteId, selectedNote])
    //#endregion effects
                            ...
                            <section className='space-y-4'>
                                ...
                                <div className='rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/40'>
                                    ...
                                    <p className='whitespace-pre-wrap break-words text-sm text-slate-700 dark:text-slate-300'>
                                        {selectedNote.content || t('No note content available')}
                                    </p>
                                </div>
                            </section>
